...
-- | Agent-deck realization: common constraints plus reported provider identity.
data DeckModelConfig = DeckModelConfig
  { deckCommonModel :: !ModelConfig,
    deckModelProvider :: !Text
  }
  deriving (Eq, Show)
...
verifyDeckModels :: DeckConfig -> [DeckModelConfig] -> IO ()
verifyDeckModels _ [] = pure ()
verifyDeckModels cfg expected = do
  ...
  where
    ...
    verify metadata configured = do
      let model = deckCommonModel configured
      require "provider" (deckModelProvider configured) (metadataProvider metadata)
      require "model" (modelName model) (metadataModel metadata)
      require "thinking" (thinkingName (modelThinking model)) (metadataThinking metadata)
      case modelMaxOutput model of
        Nothing -> pure ()
...
-- | Adapt one live agent-deck session to the engine-neutral contract.
data DeckEngine = DeckEngine DeckConfig TurnLane

-- | Adapt one live agent-deck session to the engine-neutral contract.
engineOfDeck :: DeckConfig -> IO DeckEngine
engineOfDeck cfg = DeckEngine cfg <$> newTurnLaneIO

instance Engine DeckEngine where
  startEngine (DeckEngine cfg _) context request =
    pure
      ( EngineConversation $ \extra ->
          runEngineAttempt context Nothing (engineTarget request) $ \_ ->
            deckTransport $ do
              answer <- sayDeck cfg request extra
              pure (EngineResult answer "" Unverified)
      )
  engineTurnLane (DeckEngine _ lane) = Just lane

deckTransport :: IO a -> IO a
deckTransport action =
  try action >>= \case
    Left error' ->
      throwIO
        EngineError
          { engineFailureKind = TransportFailure,
            engineFailureEvidence = renderDeckError error',
            engineFailureMessage = renderDeckError error'
          }
...
sayDeck :: DeckConfig -> EngineRequest -> Text -> IO Text
sayDeck cfg request extra = do
  dl <- deadlineIn (deckTimeoutMs cfg)
  let message = enginePrompt request <> extra
  chat cfg $
    "put "
      <> answerKindName (engineAnswerKind request)
      <> " to "
      <> engineTarget request
      <> " ("
...
-- | Optional transport narration on stderr.
