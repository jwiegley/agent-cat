...
play() {
  ...
  local mode="$1"; shift
  state="$work/$scenario"
  mkdir -p "$state/bin" "$state/config/agent-cat"
  cp engine/agent-deck/test/stub-deck.sh "$state/bin/agent-deck"
...
profiles:
  - name: deep
    chain:
      - router: configured-deck
        model: fixture-model
        thinking: high
        max-output: unconstrained
        options:
          fixture-option: enabled
EOF
  fi
  out="$state/out"
...
# deck engine, the stub and the nine scenarios above already live. It is also
...
# instances of one fixture, one per pane, and the nine scenarios above are
...
state="$work/$scenario"
mkdir -p "$state/bin"
cp engine/agent-deck/test/stub-deck.sh "$state/stub-deck.sh"
...
scenario=summary
if [ "$failures" = 0 ]; then
  echo "ci/deck: 10 scenarios passed, 0 failed"
else
