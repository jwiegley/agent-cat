...
data AcpConfig = AcpConfig
  { -- | The adapter's @argv@; the head is the program.
    acpCommand :: ![String],
    -- | Optional machine-local executable used when a bare command is absent from PATH.
    acpFallbackProgram :: !(Maybe FilePath),
    -- | Preserve every argument byte-for-byte instead of resolving path-looking arguments.
    acpLiteralArguments :: !Bool,
...
defaultAcpConfig argv =
  AcpConfig
    { acpCommand = argv,
      acpFallbackProgram = Nothing,
      acpLiteralArguments = False,
      acpCwd = ".",
...
-- | A concrete ACP command independent of CLI naming and model definitions.
data AdapterSpec = AdapterSpec
  { adapterCommand :: ![String],
    adapterFallbackProgram :: !(Maybe FilePath),
    adapterLiteralArguments :: !Bool
  }
  deriving (Eq, Show)

adapterConfig :: AdapterSpec -> [String] -> AcpConfig
adapterConfig spec arguments =
  (defaultAcpConfig (adapterCommand spec <> arguments))
    { acpFallbackProgram = adapterFallbackProgram spec,
...
-- | An adapter path or executable not known to the built-in selector packages.
pathAdapter :: Text -> AdapterSpec
pathAdapter name = AdapterSpec [T.unpack name] Nothing False

-- | Resolve the configured executable and path-valued arguments before spawn.
resolveAcpCommand :: AcpConfig -> IO [String]
resolveAcpCommand cfg = case acpCommand cfg of
  [] -> throwIO (AcpAdapterMissing "" "an empty argv names no program")
  program : arguments -> do
    resolved <- resolveProgram program
    resolvedArguments <-
      if acpLiteralArguments cfg
        then pure arguments
        else traverse absolutize arguments
    pure (resolved : resolvedArguments)
  where
    resolveProgram program
      | '/' `elem` program = absolutize program
      | otherwise = case acpFallbackProgram cfg of
          Nothing -> pure program
          Just fallback ->
            searchPath program >>= \case
              Just found -> pure found
              Nothing -> do
                there <- doesFileExist fallback
                if there
                  then pure fallback
                  else
                    throwIO . AcpAdapterMissing program $
...
permissionByIntent :: EngineIntent -> Permission
...
preflightAcpModel :: Acp -> AcpModelConfig -> IO ()
preflightAcpModel acp realization = do
  void (newSession acp)
  configureSession acp realization

configureSession :: Acp -> AcpModelConfig -> IO ()
...
desiredConfiguration acp (AcpModelConfig realization customOptions) = do
  ...
  maxOutputSetting <- case modelMaxOutput realization of
    ...
    Just wanted -> do
      ...
      pure [(option, value)]
  custom <- traverse (\(name, value) -> (,) <$> required ("option " <> name) [] [name] options <*> pure value) (Map.toList customOptions)
  let desired =
        [ (model, String (modelName realization)),
          (thinkingOption, String (thinkingName (modelThinking realization)))
        ]
          <> maxOutputSetting
          <> custom
...
steerTurn :: Acp -> EngineSteering -> Text -> IO (Either Text ())
...
instance Engine AcpEngine where
  startEngine (AcpEngine select cfg acp) context engineReq =
    acpTransport $ do
  ...
  try action >>= \case
    Left (error' :: AcpError) ->
      throwIO
        EngineError
          { engineFailureKind = TransportFailure,
            engineFailureEvidence = renderAcpError error',
            engineFailureMessage = renderAcpError error'
          }
    Right value -> pure value

-- | Put one raw engine turn. Decode, retry, failover, and completion policy are
...
sayAcpWith onChunk cfg acp engineReq extra = do
  sid <-
    readIORef (acpSession acp) >>= maybe
      (throwIO (AcpProtocol (T.pack (acpProgram acp)) "no session; nothing was opened to prompt"))
      pure
  let what = "the " <> answerKindName (engineAnswerKind engineReq) <> " question put to " <> engineTarget engineReq
      message = enginePrompt engineReq <> extra
  ...
  pure
    EngineResult
      { engineAnswer = turnText turn,
        engineNarration = turnNarration turn,
        engineCompletion =
          if stopCompleted (turnStop turn)
            then Completed
            else Incomplete (renderStopReason (turnStop turn))
      }
...
splitTransportNarration reply = peel [] (T.splitOn "\n" reply)
  where
    peel seen lines' = case lines' of
      line : rest
        | any (`T.isPrefixOf` trimAscii line) transportBanners -> peel (line : seen) rest
        | not (null seen) && T.null (trimAscii line) -> peel seen rest
      _ -> (T.intercalate "\n" (reverse seen), T.intercalate "\n" lines')

trimAscii :: Text -> Text
trimAscii = T.dropWhileEnd isWhitespaceAscii . T.dropWhile isWhitespaceAscii
  where
    isWhitespaceAscii character =
      character == ' ' || character == '\t' || character == '\r' || character == '\n'

stderrLog :: Text -> IO ()
stderrLog message = hPutStrLn stderr ("agentic: " <> T.unpack message)
