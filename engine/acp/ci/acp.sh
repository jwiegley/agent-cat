...
#
# CLI-owned model definitions cross the adapter boundary through `AcpModelConfig`:
# shared model/thinking/output constraints and one arbitrary ACP option are observed
# on the stub wire, while the sibling deck gate proves that option never reaches
# agent-deck (18).
#
...
play() {
  scenario="$1"; shift
  state="$work/$scenario"
  mkdir -p "$state/config/agent-cat"
  if [ "$scenario" = configured-options ]; then
    cat > "$state/config/agent-cat/routing.yaml" <<'EOF'
version: 1
routers:
  - name: configured-acp
    backend: acp:stub
...
#
# `engine/acp/test/acp-narrator.py` is the stub behind a proxy that writes the measured
