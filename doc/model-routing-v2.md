...
Status: implementation design; no schema or runtime change is included here.
...
The design distinguishes four names which the present schema partially conflates:
...
Sensitive-looking environment names cannot take `value`; they must take `secret`. Unknown environment-binding fields are refused. Adapter arguments remain separate and retain the existing credential-bearing argv refusal.
...
`AcpConfig` presently supplies argv, fallback executable, cwd, timeout, session policy, and verbosity; `createProcess` inherits the complete parent environment. Version 2 needs an explicit environment overlay from CLI composition.

Add a concrete-engine field representing resolved environment bindings. CLI resolves secret references immediately before spawn and passes only the resulting map to ACP. `Agentic.Engine.ModelConfig` remains unchanged.
...
- engine definitions should be able to set non-secret profile/base-directory variables explicitly;
- help and TUI text should call persona a routing context, not credential isolation; and
- deployments requiring strict separation should use distinct adapter profiles, operating-system accounts, or sandboxes.

Version 2 should not claim more isolation than the spawned adapter provides.

### 7.3 Inheritance policy

The first implementation should preserve the ambient noncredential environment needed for `PATH`, locale, home/config lookup, temporary files, proxies, and adapter operation, then overlay declared bindings. Before spawn, it should remove both every variable name declared in any configured engine's `environment` map and every source environment-variable name declared under version-2 `secrets`; it then installs only the selected engine's resolved bindings. This rule follows declared data rather than guessing from provider names or credential-shaped spellings. Undeclared ambient credentials remain one reason persona is not an isolation boundary.
...
Use `http-client` and `http-client-tls` in a CLI-owned discovery module. Do not invoke `curl`: an authorization header in argv is process-visible, executable availability is not part of the Cabal contract, and bounded body/redirect behavior would become a shell-tool contract.
...
- include a generated request id where supported, logging only its value and response status, never authorization.
...
with `~/.cache` as the XDG fallback. Each mode-0600 atomic record contains:

- cache schema version;
- persona and engine names;
- a SHA-256 fingerprint of the catalogue URL, dialect, non-secret headers, and secret reference name—not the secret value;
...
Caches are never shared across personas or differently fingerprinted engine definitions. A base URL may itself be private, so the path and routine diagnostic use its digest rather than the URL.
...
For a version-2 configured axis, changing only the backend can separate a model from the engine environment, credential, inventory, and provider through which it was validated. Version 2 should therefore override with a concrete model alias, whose declaration carries the engine:
...
Add a sanitized inspection mode:

```text
agentic-run --routing --json [--persona NAME] [--offline | --refresh-models]
```

Its JSON should contain:

- schema version and selected persona with selection source;
...
It should omit secret values, authorization/header values, adapter environment values, raw endpoint URLs by default, and model inventories not selected by any profile. An optional human rendering may show endpoint origins only on the local terminal; machine JSON uses a fingerprint.

The TUI and ext-pi consume this mode. Neither parses YAML, applies precedence, or resolves a model itself. A leading option avoids reserving a new name in the downstream workflow registry.

The machine run manifest's policy should add:
...
This is non-secret and makes a run explainable. Semantic resume compares the frozen policy exactly; a different persona, exact model, endpoint fingerprint, or settings refuses resume. Restart remains available.
...
The implementation should keep parsing, discovery, and resolution in CLI ownership:
...
These types need not be exported from `Agentic.RoutingConfig`. The public value should be a sanitized resolved policy or the existing high-level resolver result. Secret-bearing resolved environment values never receive `Eq`, `Show`, `ToJSON`, or persistence instances.
...
QuickCheck should permute map and inventory order while preserving the same resolved output.
...
Provide a mechanical command in the implementation phase:

```text
agentic-run --migrate-routing routing.yaml --output routing-v2.yaml
```

It should create one engine per v1 router, one exact concrete alias per `(router, model)` pair, one `default` persona whose engine and model allowlists contain exactly those generated names, and equivalent profile chains. It must not overwrite the source. Since v1 has no secret or endpoint data, the generated environment and catalogue fields remain absent and the result remains offline.
...
Descriptor version 3 should advertise persona/routing inspection and protocol-version negotiation. ext-pi continues to accept descriptor v2 and request protocol v1. Once updated, it reads sanitized routing JSON and passes `--persona`; it never reads secret references or model files.

### 14.4 Rollback

Version 2 implementation should leave v1 support intact. Rollback consists of restoring version-1 files and omitting `--persona`; no store conversion is necessary. Runs made under v2 preserve their frozen target policy and remain inspectable even if their persona definition is later removed.
...
1. Introduce strict v2 data types and pure decoding/authority validation beside the unchanged v1 path.
2. Add persona selection and pure resolution over supplied inventories; pin exhaustive tests before networking.
3. Add secret wrappers and ACP child-environment overlay; test sentinel non-disclosure.
4. Add bounded discovery/cache and deterministic local-server tests.
5. Extend persisted non-secret policy and semantic lineage comparison.
6. Add `--routing --json`, `--migrate-routing`, `--persona`, `--offline`, `--refresh-models`, and model-alias overrides.
7. Integrate the TUI routing pane and then ext-pi, both as consumers of sanitized output.
8. Update examples, manual, source-boundary checks, and downstream agent-workflows tests.
...
- `cli/src/Agentic/RoutingConfig.hs`, `cli/src/Agentic/Cli.hs`, `cli/model-definitions.example.yaml`, `cli/test/RoutingConfigProbe.hs`
- `engine/api/src/Agentic/Engine.hs`
- `engine/acp/src/Agentic/Acp.hs`
- `engine/agent-deck/src/Agentic/AgentDeck.hs`
- `runtime/src/Agentic/Runtime/{Protocol,Control,Store}.hs`
- `ext-pi/src/{catalogue,launch,reducer,supervisor,index}.ts` and `ext-pi/README.md`

All local paths were inspected at agent-cat revision `9df3cd3b6d42fa315b82b43db1749643cb45d9c4`.
...
Endpoint schemas and provider behavior are volatile and should be rechecked when discovery implementation begins.
