...
* **The deltas collapse to convolution** (§5.1's derivation, as a theorem).
  ...
  `List.prod` — which is what makes `List.prod` *the reducer of the
...
* Idempotence is the *duplication* licence, and at the denotation it is
  ...
  member convolved with itself is that member — needs `Std.IdempotentOp` at
  `K`'s `*`;
  `msAdd_idem` — the same alternative offered twice is offered once — needs
  `IdemSemiring S`. No general `conv f f = f` is claimed, because none is true.

Representation and denotation are bridged in one direction, explicitly.
`List.prod` is *one reducer applied to one list representation*; `panelOf`
...
theorems about lists (`List.Perm.prod_eq`, `prod_append_self`) are true and are
...
The key algebra itself is not defined here: it is Mathlib's `Monoid` and
`CommMonoid`, and `Agentic.Monoid` holds only the two reader actions that
Mathlib lacks. This module is what convolution does with the monoid.
-/
...
**SURVIVOR — what Mathlib lacks.** Mathlib's monoid semiring is
`MonoidAlgebra R M := M →₀ R` (`Mathlib/Algebra/MonoidAlgebra/Defs.lean`, line
73), and the `→₀` is the whole difficulty: a `Finsupp` is a function *together
with a proof that it is zero off a finite set*. Its convolution
(`MonoidAlgebra.mul_def`, same file, line 424) is a double `Finsupp.sum` — a
`Finset.sum` over that finite support — and the support of a product is
computed from the supports of the factors.

A panel weighting in this design is not finitely supported, and requiring it
to be would exclude the weightings the design is about. At possibility over the
free key monoid, `(fun _ => True : MSemiring Prop (List Nat))` — *any* list of
findings may come back — is nonzero at every one of infinitely many keys, and
it is a panel one asks about; so is any weighting that assigns a cost to every
transcript. Finite support is not a property these have and then lose, it is a
premise they never satisfy.

Support is also the wrong thing to *ask* for at this package's keys.
`Finsupp` stores a `Finset` of the keys where the function is nonzero
(`Mathlib/Data/Finsupp/Defs.lean`, line 87), and the convolution above is a sum over it;
the key monoid `Agentic.Trace ind` is a quotient of schedules by an inductively
generated congruence, whose equality is exactly what `acat-192` established
nobody decides. So this module aggregates with `csum` over the arbitrary index
`K × K` instead — the same trade the matrix layer makes (`Agentic.Mat` vs
`Matrix`), for the same reason, one layer up — and `total` is a `csum` over all
of `K`, which is meaningless for a `Finset.sum` and routine for a complete
semiring.

What is *not* claimed: the algebra. The laws below (`conv_assoc`,
`conv_comm`, `total_conv`, the delta collapse) are the monoid-algebra laws, and
they are re-proved here only because the carrier differs. Should Mathlib gain
a completely-supported monoid semiring over a complete semiring, this module
becomes a transport. -/
def MSemiring (S K : Type) [CommSemiring S] [CompleteCSemiring S] [Monoid K] : Type :=
  K → S
...
variable {S K : Type} [CommSemiring S] [CompleteCSemiring S] [Monoid K]
...
by coincidence: `convOne` *is* `delta 1`, and the unit of the monoid
semiring is the unit of the key monoid seen as a certain verdict. -/
theorem delta_unit : (delta 1 : MSemiring S K) = convOne := rfl
...
theorem conv_one_left (g : MSemiring S K) : conv convOne g = g := by
  funext c
  have hvanish : ∀ k1 : K, k1 ≠ 1 →
      (csum fun k2 : K =>
        if k1 * k2 = c then (if k1 = 1 then (1 : S) else 0) * g k2 else 0) = 0 :=
    fun k1 hk1 =>
      (csum_congr fun k2 => by rw [if_neg hk1, zero_mul, ite_self]).trans csum_zero
  calc conv convOne g c
      = csum (fun k1 : K => csum fun k2 : K =>
          if k1 * k2 = c then (if k1 = 1 then (1 : S) else 0) * g k2 else 0) :=
        csum_prod fun k1 k2 : K =>
          if k1 * k2 = c then (if k1 = 1 then (1 : S) else 0) * g k2 else 0
    _ = csum (fun k2 : K =>
          if (1 : K) * k2 = c then
            (if (1 : K) = 1 then (1 : S) else 0) * g k2 else 0) :=
        csum_point 1 _ hvanish
    _ = csum (fun k2 : K => if k2 = c then g k2 else 0) :=
        csum_congr fun k2 => by
          rw [one_mul, if_pos (rfl : (1 : K) = 1), one_mul]
    _ = g c :=
...
theorem conv_one_right (f : MSemiring S K) : conv f convOne = f := by
  funext c
  have hvanish : ∀ k2 : K, k2 ≠ 1 →
      (csum fun k1 : K =>
        if k1 * k2 = c then f k1 * (if k2 = 1 then (1 : S) else 0) else 0) = 0 :=
    fun k2 hk2 =>
      (csum_congr fun k1 => by rw [if_neg hk2, mul_zero, ite_self]).trans csum_zero
  calc conv f convOne c
      = csum (fun k1 : K => csum fun k2 : K =>
          if k1 * k2 = c then f k1 * (if k2 = 1 then (1 : S) else 0) else 0) :=
        csum_prod fun k1 k2 : K =>
          if k1 * k2 = c then f k1 * (if k2 = 1 then (1 : S) else 0) else 0
    ...
    _ = csum (fun k1 : K =>
          if k1 * (1 : K) = c then
            f k1 * (if (1 : K) = 1 then (1 : S) else 0) else 0) :=
        csum_point 1 _ hvanish
    _ = csum (fun k1 : K => if k1 = c then f k1 else 0) :=
        csum_congr fun k1 => by
          rw [mul_one, if_pos (rfl : (1 : K) = 1), mul_one]
    _ = f c :=
...
theorem total_one : total (convOne : MSemiring S K) = 1 := by
  refine (csum_point 1 _ fun k hk => if_neg hk).trans ?_
  rw [if_pos (rfl : (1 : K) = 1)]
...
`conv_comm`, below), and `Monoid` deliberately does not require it — an
ordered transcript is a legitimate verdict type. Against a commutative base
alone, `S⟨K⟩` could therefore be *exhibited* as a list of nine
...
exactly when the key monoid is (`conv_comm`), and `Monoid` deliberately does
...
This is the bridge the module had been missing. `List.prod` reduces a list of
...
distinction the list-level `List.Perm.prod_eq` obscures: accumulating reports in a
different order was *never* the thing commutativity of `K` was needed for. What
`CommMonoid K` licences is reordering the *convolution factors* — `conv_comm`,
...
variable {S K : Type} [CommSemiring S] [CompleteCSemiring S] [CommMonoid K]
...
carry, since Mathlib's `Semiring` deliberately does not assume `mul_comm`.

The proof is a reindexing of the pair sum by the swap: Fubini exchanges the two
aggregations, `mul_comm` matches the guards, and `mul_comm` of `S`
matches the weights.
  ...
  calc conv f g c
    ...
    _ = csum (fun k2 : K => csum fun k1 : K => if k2 * k1 = c then g k2 * f k1 else 0) :=
        csum_congr fun k2 => csum_congr fun k1 => by
          rw [mul_comm k1 k2, mul_comm (f k1) (g k2)]
    _ = conv g f c :=
...
theorem conv_delta_idem {K : Type} [CommMonoid K] [Std.IdempotentOp (α := K) (· * ·)] (k : K) :
    conv (delta k) (delta k) = (delta k : MSemiring S K) := by
  rw [conv_delta, Std.IdempotentOp.idempotent (op := (· * ·))]
...
`S` rather than to `K` — an `IdemSemiring` (possibility, worst-case cost) has
it, a counting or probabilistic carrier does not.

The licence is charged as Mathlib's bundled `IdemCommSemiring`, which is
possible only because `CompleteCSemiring` is a mixin: the two hypotheses now
name one `Semiring S`, where before this theorem had to ask for the unbundled
`Std.IdempotentOp (· + ·)` to keep the two apart. -/
theorem msAdd_idem {S K : Type} [IdemCommSemiring S] [CompleteCSemiring S] [Monoid K]
...
one *representation* a scheduler happens to hold, and `List.prod` is one
...
point mass at `List.prod`, whence the denotational forms of the two licences.
-/
...
starting from the empty verdict — is `List.prod`, and the package uses that
spelling rather than one of its own. Three of the four facts a scheduler needs
about it are already theorems:

* `List.prod_nil` — the empty panel reduces to the empty verdict;
* `List.prod_cons` — reducing a panel is combining the first member with the
  reduction of the rest;
* `List.prod_append` — a panel may be split into two and its halves reduced
  separately, so the reducer is a monoid homomorphism from lists and *sharding*
  is licensed by associativity alone;
* `List.Perm.prod_eq` — the reorder licence: at a commutative key monoid,
  members may return in any order (out of a thread pool, as network replies
  arrive, in whatever order retries settle) and the reduced verdict is
  unchanged. Without `mul_comm` this is false and the scheduler owes the design
  a fixed order. `MSemiring.convFold_perm` is the same licence about the
  weighting the list denotes.

The fourth is not in Mathlib, and is below. -/
...
Mathlib has no form of this: its idempotence lemmas about `List.prod` are about
*duplicate elements* (`List.prod_eq_pow_card` and friends assume every entry is
the same), not about a duplicated *list* under an idempotent operation, and the
idempotence hypothesis is carried unbundled because the panel's carriers arrive
as plain `Monoid`s.

A fact about the list; `MSemiring.convFold_dup` is the same licence about the
weighting. -/
theorem prod_append_self [Monoid F] (hidem : ∀ x : F, x * x = x) (l : List F) :
    (l ++ l).prod = l.prod := by
  rw [List.prod_append, hidem]

/-- The reorder licence, under the name `doc/walkthrough.html` still gives it. -/
@[deprecated (since := "2026-08-12")] alias foldPanel_perm := List.Perm.prod_eq

/-- The duplication licence, under the name `doc/walkthrough.html` still gives it. -/
@[deprecated (since := "2026-08-12")] alias foldPanel_dup := prod_append_self
...
`List.prod` reduces a list of keys and `conv` convolves weightings, and until
...
`convFold_delta` identifies it with the point mass at `List.prod`. The two
...
members is the point mass at their reduction: `⋆ᵢ δ kᵢ = δ l.prod`.
This is `conv_delta` iterated, and it is the statement that `List.prod` is not
an incidental list operation but the shadow, on point masses, of convolution
itself. -/
theorem convFold_delta (l : List K) : (convFold l : MSemiring S K) = delta l.prod := by
  induction l with
  | nil => exact delta_unit.symm
  | cons x xs ih =>
    show conv (delta x) (convFold xs) = delta (x * xs.prod)
    rw [ih, conv_delta]
...
merely the same reduced list element. This is `List.Perm.prod_eq` transported
across `convFold_delta`, and it is what a weighting can see. -/
theorem convFold_perm {S K : Type} [CommSemiring S] [CompleteCSemiring S] [CommMonoid K]
    {l l' : List K} (hp : l.Perm l') : (convFold l : MSemiring S K) = convFold l' := by
  rw [convFold_delta, convFold_delta, hp.prod_eq]
...
`prod_append_self` transported across `convFold_delta`. -/
theorem convFold_dup {S K : Type} [CommSemiring S] [CompleteCSemiring S] [CommMonoid K] [Std.IdempotentOp (α := K) (· * ·)]
    (l : List K) : (convFold (l ++ l) : MSemiring S K) = convFold l := by
  rw [convFold_delta, convFold_delta,
    prod_append_self (Std.IdempotentOp.idempotent (op := (· * ·)))]
