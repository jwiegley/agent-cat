...
-- Moving a literal into a parameter makes a formerly closed prompt depend on
-- a value supplied by its caller.
--
...
-- elaborated 'Agentic.DSL.Plan.Plan', the bills and the generated names are
...
input :: Text -> In
input n = In (InputSpec n PromptInput)
...
sourcedInput _ source n = In (InputSpec n source)
