...
`lake exe corpus-gen`, from the `bisim/` module root.
...
from them.** Every file under `corpus/` already holds a `request` that is
...
a no-op.** `lake exe corpus-gen` followed by `git status --short bisim/corpus`
