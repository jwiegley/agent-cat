...
-- (@bisim/corpus/*.json@ at the repository root) against this
...
--
-- The expected tally is the whole of @bisim\/corpus@: every @string@ entry,
...
-- @bisim/corpus@ from the repository root. Exit status is 0 iff nothing
-- failed.
{-# LANGUAGE LambdaCase #-}
...
import Agentic.Bisim (Guard (..), askCounts, guardCheck)
import Agentic.DSL (RawProgram, stringOpOf)
