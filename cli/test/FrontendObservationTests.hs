...
checkCapabilities = do
  ...
  expect "explicit control-observation session advertisement" (capabilitySessionVersions caps == [1,2])
  expect "other capability versions unchanged" (capabilityExportVersions caps == [1] && capabilityInvocationVersions caps == [1])
