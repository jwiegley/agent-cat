...
import Agentic.Runtime.Facts (runFactName)
...
-- | A 'runFactName' under the runner's own prefix, misspelled.
--
...
inputIsMisspelledRunFact =
  ...
  where
    par = taking (input (runFactName "run.backend") :> noInputs) \b -> workflow W.do
