...
main = do
  ...
  let delayedDir = temp </> ("agentic-delayed-permission-" <> show stamp)
      delayedCfg =
        (adapterConfig stubAdapter ["--delayed-same-session-permission"])
  ...
  delayed <-
    try
      ( withAcp delayedCfg $ \acp -> do
          _ <- sayAcp delayedCfg acp (engineRequest SAck delayedRequest) ""
          _ <- newSession acp
  ...
  resolvedDroid <- resolveAcpCommand (adapterConfig droidAdapter ["./test/PolicyProbe.hs"])
  ...
  pureProbe failures "adapter selection keeps Droid's native ACP mode"
    [ ("droid", adapterCommand droidAdapter == ["droid", "exec", "--output-format", "acp"]),
      ("appended argv", adapterCommand droidAdapter <> ["--probe"] == ["droid", "exec", "--output-format", "acp", "--probe"])
    ]
