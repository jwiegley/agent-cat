...
import Agentic.Acp (AcpConfig (..), childEnvironmentNames, defaultAcpConfig, withAcp)
...
sharedAcpYaml =
  BS.unlines
    [ "version: 2",
      ...
      "models:",
      "  first-model: {engine: first, select: [{exact: first-model}]}",
      "  second-model: {engine: second, select: [{exact: second-model}]}",
      "personas:",
      "  test:",
      ...
      "    profiles:",
      "      primary:",
      "        chain: [{model: first-model, thinking: high, max-output: 4096}]",
      "      spare:",
      "        chain: [{model: second-model, thinking: high, max-output: 4096}]"
    ]

deckEnvironmentYaml :: BS.ByteString
deckEnvironmentYaml =
  BS.unlines
    [ "version: 2",
      "default-persona: test",
      ...
      "    provider: openai",
      "    environment:",
      "      SHOULD_NOT_BE_ACCEPTED: {value: ignored}",
      "models:",
      "  pane-model: {engine: pane, select: [{exact: pane-model}]}",
      "personas:",
      "  test:",
      "    engines: [pane]",
      "    models: [pane-model]",
      "    profiles:",
      "      primary:",
      "        chain: [{model: pane-model, thinking: high, max-output: 4096}]"
    ]

publicCatalogueYaml :: String -> BS.ByteString
publicCatalogueYaml host =
  BS.unlines
    [ "version: 2",
      "default-persona: test",
      "secrets: {}",
      ...
      "    catalogue:",
      "      dialect: openai",
      BS.pack ("      url: http://" <> host <> "/v1/models"),
      "      timeout-ms: 5000",
      "      max-bytes: 4194304",
      "      cache: {fresh-for: 1h, stale-if-error: 24h}",
      "models:",
      "  public-model: {engine: public, select: [{exact: public-model}]}",
      "personas:",
      "  test:",
      "    engines: [public]",
      "    models: [public-model]",
      "    profiles:",
      "      primary:",
      "        chain: [{model: public-model, thinking: high, max-output: 4096}]"
    ]
...
main = do
  ...
  case decodeRoutingUserV2 sharedAcpYaml >>= (\config -> selectRoutingPersona config Nothing Nothing Nothing) of
    Left problem -> check failures ("same-backend engine instances decode: " <> problem) False
    Right selected ->
      check failures "same ACP backend retains distinct engine environments" $
        case resolveEngineContexts selected ["first", "second"] Map.empty of
          Right contexts ->
            fmap (childEnvironmentNames . resolvedEngineChildEnvironment) (Map.lookup "first" contexts) == Just (Just ["INSTANCE_FIRST"])
              && fmap (childEnvironmentNames . resolvedEngineChildEnvironment) (Map.lookup "second" contexts) == Just (Just ["INSTANCE_SECOND"])
              && fmap resolvedEngineBackend (Map.lookup "first" contexts) == Just (BackendAcp "codex")
              && fmap resolvedEngineBackend (Map.lookup "second" contexts) == Just (BackendAcp "codex")
          Left _ -> False
  check failures "deck engines reject unusable child environment bindings" $
    isLeftContaining "cannot declare child environment" (decodeRoutingUserV2 deckEnvironmentYaml)
  check failures "only literal well-formed 127/8 addresses receive loopback HTTP" $
    either (const False) (const True) (decodeRoutingUserV2 (publicCatalogueYaml "127.0.0.1"))
      && all
        (isLeftContaining "literal loopback" . decodeRoutingUserV2 . publicCatalogueYaml)
        ["127.999.999.999", "127.....", "127.0.0", "127.0.0.256"]
        ...
        Right selected -> do
          ...
          case Map.lookup "personal-claude" =<< either (const Nothing) Just contexts of
            ...
            Just context -> do
              ...
              check failures "child environment scrubs every declared source and destination before selected overlay" $
                case names of
                  Just values ->
                    "ANTHROPIC_API_KEY" `elem` values
                      && "PERSONAL_API_KEY" `notElem` values
                      && "WORK_API_KEY" `notElem` values
                      && "OPENAI_API_KEY" `notElem` values
                      && "KEEP_ME" `elem` values
                  Nothing -> False
  ...
  check failures "v2 user and project layers load without becoming legacy routing" $
    case loadedV2 of
      Right loaded ->
        Map.null (routingProfiles (loadedRouting loaded))
          && fmap routingV2DefaultPersona (loadedRoutingV2User loaded) == Just "personal"
          && fmap projectRoutingPersona (loadedRoutingV2Project loaded) == Just "personal"
          && loadedRoutingSources loaded == [userPath, projectPath]
      Left _ -> False
