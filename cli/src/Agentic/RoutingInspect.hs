      ...
      Just engine ->
        object
          [ "name" .= name,
            ...
            "launch"
              .= object
                [ "targetKind" .= backendTargetKind (engineBackend engine),
                  "arguments" .= backendLaunchArguments (engineBackend engine),
                  "fingerprint" .= routingLaunchFingerprint (selectedPersonaName selected) (engineBackend engine) resolved
                ],
...
launchRealizationPolicy :: ResolvedRealization -> Value
launchRealizationPolicy target =
  object
    ( [ "profile" .= resolvedProfile target,
        ...
        "router" .= routerName (resolvedRouter target),
        "provider" .= routerProvider (resolvedRouter target),
        "model" .= realizationModel (resolvedSpec target),
        "thinking" .= thinkingName (realizationThinking (resolvedSpec target)),
        "maxOutput" .= realizationMaxOutput (resolvedSpec target),
        "options" .= realizationOptions (resolvedSpec target)
      ]
        <> maybe [] (\fingerprint -> ["executionFingerprint" .= fingerprint]) (resolvedExecutionFingerprint target)
        <> case resolvedModelSelection target of
          Nothing -> []
          Just selection ->
            [ "modelAlias" .= selectedModelAlias selection,
              ...
              "selectorIndex" .= selectedModelSelectorIndex selection,
              "selector" .= selectorJson (selectedModelSelector selection),
              "inventoryFingerprint" .= selectedModelFingerprint selection
            ]
    )

backendTargetKind :: Backend -> Text
backendTargetKind BackendAcp {} = "acp"
backendTargetKind BackendDeck {} = "deck"

backendLaunchArguments :: Backend -> [Text]
backendLaunchArguments (BackendAcp adapter) = ["--engine", "acp", "--adapter", adapter]
backendLaunchArguments (BackendDeck session) = ["--session", session]
...
firstText :: Either Text a -> Either Text a
firstText = id
