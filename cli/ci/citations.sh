#!/usr/bin/env bash
#
# The citation gate — every `<File>.lean:<N>` in live Haskell modules resolves.
#
...
# docstring across the modular Haskell workspace:
#
...
# Every citation written in live Haskell modules, as
...
find dsl plan cost runtime engine workflow bisim/haskell cli \
  \( -name '*.hs' -o -name '*.md' -o -name '*.sh' \) \
...
# changed under it. The modular live-source inventory is 197; 190 keeps the
# floor close enough to catch a broken scan while allowing genuine citation cleanup.
if [ "$total" -lt 190 ]; then
  echo "ci/citations: only $total citations found — below the 190 floor; the scan is broken, not the tree" >&2
  exit 1
fi
