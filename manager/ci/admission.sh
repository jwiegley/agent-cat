...
for capabilities in N1 N8; do
  mkdir "$work/$capabilities" "$work/shutdown-only-$capabilities" "$work/shutdown-native-$capabilities" "$work/restart-$capabilities"
  "$runner" restart-native "$work/restart-$capabilities" "$native" +RTS "-$capabilities" -RTS 2>&1 | tee "$work/restart-$capabilities.log"
