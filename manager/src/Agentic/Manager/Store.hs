...
module Agentic.Manager.Store
  ( CoordinationStore, StoreIdentity (..), StoreFailure (..), Checkpoint (..),
    ...
    Transaction, execute, query, refuseTransaction, runTransaction, runRead,
    Invalidation (..)
  ) where

import Agentic.Manager.Configuration
  (InstalledConfiguration, acquireConfigurationStorage, releaseConfigurationStorage)
...
data CoordinationStore = CoordinationStore !PrivateRoot !SQL.Database !StoreIdentity
...
data Context = Context !SQL.Database !Bool !(IORef Budget) !(IORef Bool)
...
withCoordinationStore installed action = do
  ...
  bracket (acquireConfigurationStorage installed) release $ \(root, _) ->
    bracket (openStore root) closeStore action
  where
    release (root, lease) =
      (closePrivateRoot root `finally` closeFd lease) `finally` releaseConfigurationStorage installed

openStore :: PrivateRoot -> IO CoordinationStore
openStore root = storageErrors $ do
  ...
  bracketOnError (SQL.open2 (T.pack (privateRootPath root </> databaseName))
    ...
    (epoch, stream) <- bounded db 30000000 $ do
      ...
      unless (version == SQL.SQLInteger 0 || version == SQL.SQLInteger (fromIntegral schemaVersion)) $
    ...
    CoordinationStore root db (StoreIdentity schemaVersion epoch stream generation)
...
closeStore (CoordinationStore _ db _ gate closed poisoned) = uninterruptibleMask_ $ do
...
storeIdentity store@(CoordinationStore _ _ identity _ _ _) = admitted store (pure identity)

checkpointStore :: CoordinationStore -> IO Checkpoint
checkpointStore store@(CoordinationStore _ db _ _ _ _) = admitted store $ bounded db 5000000 $ do
  verifyPragmas db
...
admitted (CoordinationStore root _ _ gate closed poisoned) action = mask $ \restore -> do
...
execute sql parameters = Transaction $ \context@(Context db writable _ changed) -> do
...
query sql parameters = Transaction $ \context@(Context db _ budget _) -> do
...
refuseTransaction :: StoreFailure -> Transaction a
...
run store@(CoordinationStore _ db _ _ _ poisoned) writable (Transaction action) = admitted store $ do
  ...
  mask $ \restore -> do
    result <- try @SomeException $ restore $ bounded db 5000000 $ do
      ...
      (resultValue, events) <- action (Context db writable budget changed)
...
chargeInput (Context _ _ budget _) sql parameters = do
