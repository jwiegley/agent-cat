...
module Agentic.Manager.Service
  ( Service, ServiceFault (..), withService, serviceStore, serviceFault,
    enqueue, approve, controlRun, controlDecision, readControl,
    withSnapshot, withOutputs, download
  ) where
...
import Agentic.Manager.Authorization (CredentialProof, AuthorizedView)
...
import Agentic.Runtime (FrontendPrepared (..), RunSnapshot)
  ...
  where
    loop = do
      ...
      unless closing $ do
        ...
        case event of
          Nothing -> pure ()
          Just changed -> when changed (fill service) >> loop

-- Each notification denotes new admitted queue or released-reservation facts.
-- An opaque failed admission is recorded, not retried on the polling timer and
-- not converted into global worker cancellation. Other owned runs keep ingesting.
fill :: Service -> IO ()
fill service = do
  room <- atomically $ do
  ...
  when room $ do
    outcome <- try @SomeException (A.admitOldest (admission service))
    case outcome of
      Left failure | Just asynchronous <- (fromException failure :: Maybe SomeAsyncException) -> throwIO asynchronous
      Left failure -> atomically (writeTVar (faultCell service) (Just (fault failure)))
      Right (Left failure) -> atomically (writeTVar (faultCell service) (Just (CommandFault failure)))
      Right (Right next) -> do
        atomically (writeTVar (faultCell service) Nothing)
        forM_ next $ \live -> startLoan service live >> fill service
...
readControl service proof ident = do
  association <- State.resolveRun (serviceStore service) proof [Observe] ident
  original <- startFor service ident
  case original of
    Just start -> State.readControlSurface start proof
    Nothing -> State.readClosedControlSurface (serviceStore service) proof association

withSnapshot :: Service -> CredentialProof -> Text -> (AuthorizedView -> RunSnapshot -> IO a) -> IO a
withSnapshot service proof ident respond = do
  association <- State.resolveRun (serviceStore service) proof [Observe] ident
  State.withProfileProjection (serviceStore service) proof association respond
...
download :: Service -> CredentialProof -> Text -> (AuthorizedView -> Value -> BS.ByteString -> IO ()) -> IO ()
