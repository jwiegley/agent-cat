...
main :: IO ()
main = do
  captureFaultBucketEvidenceData
  temporary <- getTemporaryDirectory
  withCaptureBucket "agentic-capture-fault-" temporary $ \bucket -> do
    ...
    withPrivateRoot "capture fault root" path $ \root -> do
      ...
      check "retry after uncertain sync neither deletes nor replaces" $ before == after && case retry of
        ...
        _ -> False
      roleFaultTests bucket
  putStrLn "capture and role fault tests passed: real barriers, injected sync errors, post-link replacement and cancellation"

captureFaultBucketEvidenceData :: IO ()
captureFaultBucketEvidenceData = do
  bucketEvidenceChecks "agentic-capture-fault-"
  putStrLn "PASS capture fault bucket evidence data-only checks"
