...
import Agentic.Planning (Addressee (AddrToolExec), Code, decode)
    ...
    abandon n out err =
      ioError . userError . T.unpack $
        "the command `"
          ...
          <> ", and a "
          <> codeWord (fromSCode c)
          <> " has no value that says so: the run is abandoned rather than recording an answer indistinguishable, in the table, from one a command that succeeded gave. (prompt: '"
          <> oneLine (qPrompt (reqQuestion q))
          <> "'; it said: '"
          <> oneLine (T.take 400 (err <> out))
          <> "'). Ask it as a `flag` or a `verdict` if a nonzero exit is an answer here."
