...
worldOfEngineWith settings engine =
  WorldIO
    { worldAskIO = engineAsk Nothing,
      ...
      worldTurnLane = \_ _ -> engineTurnLane engine
    }
  where
...
engineRequest code request =
  EngineRequest
    { engineTarget = addresseeWord (qAddressee question),
      engineModelAxis = scopeModelAxis (qScope question),
      engineModeAxis = scopeModeAxis (qScope question),
      engineDraw = qDraw question,
      engineIntent = case reqIntent request of
        Consult -> Engine.Consult
        Observe -> Engine.Observe
        Effect -> Engine.Effect,
      engineAnswerKind = case code of
        SText -> Engine.TextAnswer
        SVerdict -> Engine.VerdictAnswer
        SFlag -> Engine.FlagAnswer
        SAck -> Engine.ReceiptAnswer
        SStructured _ -> Engine.StructuredAnswer,
      enginePrompt = renderRequest code request,
      engineRequiresCompletedTurn = requiresCompletedTurn request
    }
  where
  ...
  when (not (T.null (engineNarration result))) $
    esLog settings $
      "transport narration separated from the answer to "
        <> what
        <> ": '"
        <> oneLine (engineNarration result)
        <> "' — the adapter's own words about itself, kept out of the answer, which is what the trusted base reads and what every later prompt quotes"
  case engineCompletion result of
    Completed -> pure answer
    Unverified -> pure answer
    Incomplete reason -> do
      esLog settings $
        "turn for a "
          <> codeWord (fromSCode code)
          <> " from "
          <> engineTarget neutral
          <> " ended '"
          <> reason
          <> "', not 'end_turn'"
      if engineRequiresCompletedTurn neutral
        then
          throwIO
            EngineError
              { engineFailureKind = ProtocolFailure,
                engineFailureEvidence = "the turn ended '" <> reason <> "'",
                engineFailureMessage =
                  "the turn that would have answered a "
                    ...
                    <> "'; what arrived: '"
                    <> trimAscii answer
                    <> case reqIntent request of
                      Effect -> "'). The run is abandoned: an unfinished turn did not perform the act it was asked to perform, and a recorded acknowledgement of it would be indistinguishable, in the table, from one that did."
                      _ -> "'). The run is abandoned: the unfinished stand-in did not obtain the person's answer, so recording one would attribute words that person never gave."
              }
        else pure answer

engineGap :: SomeException -> Maybe (TurnGap, Text)
engineGap exception = do
  EngineError kind evidence _ <- fromException exception
  pure
    ( case kind of
        TransportFailure -> GapTransportRefusal
        ProtocolFailure -> GapEmptyOrProtocol,
      evidence
    )
...
askOrMemo scheduler w ch context c q = do
  ...
  where
    ...
    spentSkip qi =
      fromMaybe (addresseeWord (qAddressee (reqQuestion qi))) (modelOf qi)
        <> " reported its allowance spent earlier in this run; not asking it again"
...
abandonAllSpent c q chain =
  ioError . userError . T.unpack $
    "no model is left to answer the "
      ...
      <> oneLine (qPrompt q)
      <> "'). The run is abandoned rather than asking a model known to be spent, which would be spending a turn on a guess."
      ...
      (Just arm, SFlag) -> do
        esLog st $
          "no readable flag from "
            ...
            <> sayFlag arm
            <> ") rather than abandoning the run — nothing checks that this arm is the loud one; that safety is the operator's. This answer is not "
            <> addresseeWord (qAddressee (reqQuestion q))
            <> "'s (last reply: '"
            <> trimAscii (gaWhy ga)
            <> "', prompt: '"
        ...
        let final =
              userError . T.unpack $
                "no readable "
                  ...
                  <> "'). The run is abandoned: recording an answer nobody gave would be indistinguishable, in the table, from one they did."
            why =
              "no readable "
                <> codeWord (fromSCode c)
                <> " after "
